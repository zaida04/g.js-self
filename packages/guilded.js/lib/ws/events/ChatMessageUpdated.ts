...
export default class ChatMessageUpdatedEvent extends Event {
    ...
    public ingest(data: WSChatMessageUpdated): (boolean | (string | undefined))[] {
        if (data) {
            const channel = this.client.channels.cache.get(data.channelId);
            const oldMessage = channel?.messages?.cache.get(data.message.id);
            if (!oldMessage && !this.client.options?.partials?.includes('MESSAGE')) {
                return [false, 'Old message not cached!'];
            }
            const newMessage =
                oldMessage?._clone().patch(data.message) ??
                new PartialMessage(this.client, { ...data.message, channelId: data.channelId });
